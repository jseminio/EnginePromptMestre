...
./ai_orchestrator_suite_v3/devops/ai.sh init

# Executar orquestrador com defaults (engine=auto, history=strict, decision=DE ACORDO)
./ai_orchestrator_suite_v3/devops/ai.sh run orchestrator tasks/FEAT-101.md

# Forçar failover completo e modo automático
env DECISION_MODE=AUTOMÁTICO ./ai_orchestrator_suite_v3/devops/ai.sh run backend tasks/BUG-202.md --engine=abc --history=strict
```

No Windows PowerShell:
```powershell
PS> .\ai_orchestrator_suite_v3\devops\ai.ps1 init
PS> .\ai_orchestrator_suite_v3\devops\ai.ps1 run orchestrator tasks\FEAT-101.md --engine=auto --decision="DE ACORDO"
```
