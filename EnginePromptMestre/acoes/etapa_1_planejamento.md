...
4. Verificar alternativas existentes (reuso ≥80%) para cada objetivo/artefato e registrar justificativas quando criar algo novo.  
5. Traçar estratégia de entrega (fases/trilhas, dependências).  
6. Mapear artefatos (criar/modificar/ler) com justificativa de reuso.  
7. Definir reuso, eliminar duplicidades, planejar feature flags + rollback.  
8. Planejar testes/métricas (unit, integração, regressão, performance, observabilidade).  
9. Salvar `acoes/temp/contexto_etapa_1.json` + atualizar `sessao_atual.json` (`etapa_atual=1`, `proxima_etapa=2`).
